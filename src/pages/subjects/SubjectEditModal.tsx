...
export function SubjectEditModal({ subjectId, isOpen, onClose, onSuccess }: SubjectEditModalProps) {
  const [isFetching, setIsFetching] = useState(false)
  const [isSubmitting, setIsSubmitting] = useState(false)
  const isEditMode = !!subjectId && subjectId !== 'new'

  const { register, handleSubmit, reset, formState: { errors } } = useForm<CreateSubjectInput>()

  useEffect(() => {
    if (isOpen && subjectId && subjectId !== 'new') {
      setIsFetching(true)
    ...
    } else if (isOpen && (subjectId === null || subjectId === 'new')) {
      reset({
        name: "",
        code: "",
      })
